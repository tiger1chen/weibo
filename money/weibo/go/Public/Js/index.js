$(function(){
	...
	$('#picture').uploadify({
		...
		onUploadSuccess : function (file, data, response) {
			eval('var data='+data);
			if(data.status == 1){//返回数据成功
				$('input[name=max]').val(data.path.max);
				$('input[name=medium]').val(data.path.medium);
				$('input[name=mini]').val(data.path.min);
				
				$('#upload_img').fadeOut().next().fadeIn().find('img').attr('src',ROOT +  data.path.medium);
			}else{
				alert('发布微博图片失败');
			}
